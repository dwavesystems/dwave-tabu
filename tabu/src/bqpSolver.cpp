...
BQPSolver::BQPSolver(BQP b) : bqp(b) {}

BQP BQPSolver::multiStartTabuSearch(long long timeLimitInMilliSecs, 
                                    int numStarts, 
                                    int tabuTenure, 
...
double BQPSolver::simpleTabuSearch(const vector<int> &starting,
...
double BQPSolver::localSearchInternal(const vector<int> &starting, double startingObjective, vector<double> &changeInObjective) {
...
void BQPSolver::selectVariables(int numSelection, vector<vector<double>> &C, vector<int> &I) {
    int i, ctr;
...
void BQPSolver::steepestAscent(int numSelection, vector<vector<double>> &C, vector<int> &I, vector<int> &solution) {
    int i, j, ctr;
    int idI, idJ, r, v = 0;
...
void BQPSolver::computeC(vector<vector<double>> &C, const vector<int> &solution) {
    for (int i = 0; i < bqp.nVars; i++) {
        C[i][i] = -bqp.Q[i][i];
